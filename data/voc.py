...
class VOC_seg(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transforms != None:
            ...
            #kshape = ykuang.shape
            #palette = [[0], [1]]
            #ykuang = np.reshape(ykuang, (kshape[0], kshape[1], 1))
            #ykuang_ = mask_to_onehot(ykuang, palette)
            #img, masks, ykuang_ = self.transforms(img, masks, ykuang_)
           img, masks = self.transforms(img, masks)

        
        #return img, masks, fn, ykuang_
        return img, masks, fn
