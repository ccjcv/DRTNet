...
class Compose():
    ...
    def __call__(self, img, masks, kuang):
        for tr in self.transforms:
            img, masks, kuang = tr(img, masks, kuang)
        return img, masks, kuang
...
class RandomScale():
    ...
    def __call__(self, img, masks, kuang):
        ...
        kuang = cv2.resize(kuang, None, fx=scale, fy=scale, interpolation=cv2.INTER_NEAREST)
        return img, new_masks, kuang
...
class ResizeRandomCrop():
    ...
    def __call__(self, img, masks, kuang):
        ...
        for mask in masks:
            ...
            new_masks.append(mask)
        kuang = cv2.copyMakeBorder(kuang, padH, padH, padW, padW, cv2.BORDER_CONSTANT, value=0)
        kuang = kuang[new_hmin:new_hmin + crop_H, new_wmin:new_wmin + crop_W]
        return img, new_masks, kuang
...
class Normalize_Caffe():
    ...
    def __call__(self, img_RGB, masks, kuang):
        ...
        for mask in masks:
            ...
            new_masks.append(mask)
        kuang = torch.from_numpy(kuang).float()
        return img, new_masks, kuang