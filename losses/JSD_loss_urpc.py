...
def calc_jsd_multiscale(labels1_a, pred1, pred2, pred3, weight, threshold=0.8):
    ...
    preds = probs[0] * weight_softmax[0] + probs[1] * weight_softmax[1] + probs[2] * weight_softmax[2]
    ...
    log_probs = [torch.sum(F.kl_div(torch.log(preds), prob, reduction='none') * mask, dim=1, keepdim=True) for prob in
                 probs]
