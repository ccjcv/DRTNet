...
def calc_jsd_multiscale_2(weight, labels1_a, pred1, pred2, pred3, threshold=0.8, Mask_label255_sign='no'):
    ...
    #logp_mixture, prob
    log_probs = [torch.sum(F.kl_div(torch.log(mixture_label), prob, reduction='none') * mask, dim=1, keepdim=True) for prob in probs]

    # exp_variance_1 = torch.exp(-log_probs[0])
    # exp_variance_2 = torch.exp(-log_probs[1])
    # exp_variance_3 = torch.exp(-log_probs[2])
    # consistency_dist_1 = (mixture_label - probs[0]) ** 2
    # consistency_dist_2 = (mixture_label - probs[1]) ** 2
    # consistency_dist_3 = (mixture_label - probs[2]) ** 2
    #
    ...
    return torch.mean(loss), consistency_loss, mixture_label