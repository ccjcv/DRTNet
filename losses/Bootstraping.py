...
class BootstrapingLoss(nn.Module):
    
    def __init__(self, num_classes, lambda_wgt):
        super(BootstrapingLoss, self).__init__()
        self.num_classes = num_classes
        self.lambda_wgt = lambda_wgt
        self.soft_max = nn.Softmax(dim=1)
        
    def forward(self, y_pred, ycrf, yret, beta):
        y_pred = self.soft_max(y_pred)
        loss_ce = self.get_loss_ce(y_pred, ycrf, yret)
        loss_bs = self.get_loss_bs(y_pred, ycrf, yret, beta)
        loss = loss_ce + self.lambda_wgt * loss_bs
        return loss
    ...
    def get_loss_bs(self, y_pred, ycrf, yret, beta):
        n_classes_arr=torch.arange(self.num_classes).cuda()
        ...
        y_crf_encoded = torch.eye(self.num_classes)[ycrf]
        y_crf_encoded = y_crf_encoded.permute(0, 3, 1, 2)
        y_crf_encoded = y_crf_encoded.cuda()
        
        for i in range(self.num_classes):
            t = not_s_class[:,i,:,:]
            Y = (beta * y_crf_encoded[:,i,:,:][t]) + (1 - beta) * (y_pred[:,i,:,:][t])
            numer += torch.sum(Y * torch.log(y_pred[:,i,:,:][t]))
