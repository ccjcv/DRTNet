...
from models.mit_amm import mit_amm_backbone
...
class Labeler(nn.Module):
    def __init__(self, num_classes, roi_size, grid_size):
        super().__init__()
        #这部分是两路resnet50
        self.mit_backbone = mit_backbone()
        ...
        # self.resnet50_2 = resnet50.resnet50(pretrained=True, use_amm=True, strides=(2, 2, 2, 2))
        ...
        #
        # self.fc_2 = nn.Conv2d(2048, 512, 1)
        # self.dropout1 = nn.Dropout2d(0.5)
        self.mit_amm_backbone = mit_amm_backbone()

        self.classifier2 = nn.Conv2d(512, num_classes, 1, bias=False)
    ...
    def get_features(self, x):
        ...
        # x = self.stage3(x)
        # x = self.stage4(x)
        x = self.mit_backbone(x)#(8,512,16,16)

        # # branch2
        # x2 = self.stage2_1(x_ori)
        # x2 = self.stage2_2(x2)
        # x2 = self.stage2_3(x2)
        # x2 = self.stage2_4(x2)#(8,2048,16,16)
        # x2 = F.relu(self.fc_2(x2))#(8,512,16,16)
        # x = self.dropout1(x)
        x2 = self.mit_amm_backbone(x_ori)
    ...
    def BAP(self, features, bboxes, batchID_of_box, bg_protos, bg_protos_img,
                    valid_cellIDs, ind_valid_bg_mask, GAP=False):
            ...
            for coord, width, height in zip(bboxes[indices], widths[indices], heights[indices]):
                ...
                if GAP:
                    ...
                    if ind_valid_bg_mask[batch_id] and len(ID_list):
                        ...
                        valid_bg_p_img = bg_protos_img[batch_id, ID_list]
                        valid_bg_p_img_he = torch.sum(valid_bg_p_img, dim=1)
                        kuang_num = valid_bg_p_img_he.size(0)
                        yuzhi = torch.tensor(-330)#shiyongde -330  #-342
                        black_shu_he = torch.tensor([])
                        for i in range(kuang_num):
                            if valid_bg_p_img_he[i][0][0] < yuzhi:
                                black_shu_1 = torch.tensor([i])
                                black_shu_he = torch.cat((black_shu_he, black_shu_1), 0)
                        geshu = black_shu_he.size(0)
                        if geshu == kuang_num:
                            geshu = 0

                        normed_bg_p = F.normalize(valid_bg_p, dim=1)
                        bg_attns = F.relu(torch.sum(normed_roi * normed_bg_p, dim=1, keepdim=True))
                        a, b, c, d = bg_attns.size()
                        bg_attns_new = bg_attns.clone()
                        for i in range(geshu):
                            bg_attns_new[int(black_shu_he[i]), :, :, :] = torch.tensor(-1)
                        valid_row = []
                        for row_idx in range(bg_attns_new.size(0)):
                            if not torch.all(bg_attns_new[row_idx, :, :, :] == -1):
                                valid_row.append(row_idx)
                        bg_attns_new_2 = bg_attns_new[valid_row, :, :, :]
                        #youwu beijingxianyan jiushi  bg_attns_new_2 he bg_attns
                        bg_attn = torch.mean(bg_attns_new_2, dim=0, keepdim=True)
                        fg_attn = 1 - bg_attn
    ...
    def forward(self, img, bboxes, batchID_of_box, bg_mask, ind_valid_bg_mask, GAP=False):
        ...
        img_resize = Resize([fH, fW])
        img_ori = img_resize(img)
        padded_img = pad_for_grid(img_ori, self.GS)
        ##########################################################
        ...
        bg_protos_img = self.get_bg_prototypes(padded_img, padded_mask, grid_bg, self.GS)
        bg_protos = self.get_bg_prototypes(padded_features, padded_mask, grid_bg, self.GS)
        fg_protos = self.BAP(features, bboxes, batchID_of_box, bg_protos, bg_protos_img,
        ...
        fg_protos_2 = self.BAP(features_2, bboxes, batchID_of_box, bg_protos_2, bg_protos_img,
                               valid_cellIDs_2, ind_valid_bg_mask, GAP)
        ##########################################################
