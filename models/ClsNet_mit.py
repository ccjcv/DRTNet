...
from models.mit import mit_backbone
...
class Labeler(nn.Module):
    ...
    def forward(self, img, bboxes, batchID_of_box, bg_mask, ind_valid_bg_mask, GAP=False):
        ...
        '''
        #yuan lai de
        # features = self.get_features(img)  # (8,512,16,16)
        #
        # batch_size, dims, fH, fW = features.shape
        # ##########################################################
        # padded_mask = pad_for_grid(F.interpolate(bg_mask, (fH, fW)), self.GS)#(8,1,16,16)
        # grid_bg, valid_gridIDs = self.get_grid_bg_and_IDs(padded_mask, self.GS)#8
        # valid_cellIDs = []
        ...
        # bg_protos = self.get_bg_prototypes(padded_features, padded_mask, grid_bg, self.GS)#(8,16,512,1,1)
        # fg_protos = self.BAP(features, bboxes, batchID_of_box, bg_protos, valid_cellIDs, ind_valid_bg_mask, GAP)#(8,512,2,2)
        # ##########################################################
        # num_fgs = fg_protos.shape[0]#8
        # fg_protos = fg_protos.view(num_fgs, dims, -1).permute(0, 2, 1).contiguous().view(-1, dims, 1,
        #                                                                                  1) #(32,512,1,1) # (num_fgs,dims,OH,OW) --> (num_fgs*OH*OW,dims,1,1)
        # bg_protos = bg_protos.contiguous().view(-1, dims, 1, 1)[valid_gridIDs]  #(117,512,1,1) (len(valid_gridIDs),dims,1,1)
        # protos = torch.cat((fg_protos, bg_protos), dim=0)#(149,512,1,1)

        #gai jin de
        f_1 = self.get_features(img)  # (8,512,16,16)

        batch_size, dims, fH, fW = f_1.shape
        ##########################################################
        padded_mask_1 = pad_for_grid(F.interpolate(bg_mask, (fH, fW)), self.GS)  # (8,1,16,16)
        grid_bg_1, valid_gridIDs_1 = self.get_grid_bg_and_IDs(padded_mask_1, self.GS)  # 8
        valid_cellIDs_1 = []
        for grids in grid_bg_1:
            valid_cellIDs_1.append([idx for idx, cell in enumerate(grids) if cell > 0])
        ##########################################################
        padded_features_1 = pad_for_grid(f_1, self.GS)
        bg_protos_1 = self.get_bg_prototypes(padded_features_1, padded_mask_1, grid_bg_1, self.GS)  # (8,16,512,1,1)
        fg_protos_1 = self.BAP(f_1, bboxes, batchID_of_box, bg_protos_1, valid_cellIDs_1, ind_valid_bg_mask,
                             GAP)  # (8,512,2,2)
        ##########################################################
        num_fgs_1 = fg_protos_1.shape[0]  # 8
        fg_protos_1 = fg_protos_1.view(num_fgs_1, dims, -1).permute(0, 2, 1).contiguous().view(-1, dims, 1,
                                                                                         1)  # (32,512,1,1) # (num_fgs,dims,OH,OW) --> (num_fgs*OH*OW,dims,1,1)
        bg_protos_1 = bg_protos_1.contiguous().view(-1, dims, 1, 1)[
            valid_gridIDs_1]  # (117,512,1,1) (len(valid_gridIDs),dims,1,1)
        protos_1 = torch.cat((fg_protos_1, bg_protos_1), dim=0)  # (149,512,1,1)

        # batch_size, dims, fH, fW = f_2.shape
        # ##########################################################
        # padded_mask_2 = pad_for_grid(F.interpolate(bg_mask, (fH, fW)), self.GS)  # (8,1,16,16)
        # grid_bg_2, valid_gridIDs_2 = self.get_grid_bg_and_IDs(padded_mask_2, self.GS)  # 8
        # valid_cellIDs_2 = []
        # for grids in grid_bg_2:
        #     valid_cellIDs_2.append([idx for idx, cell in enumerate(grids) if cell > 0])
        # ##########################################################
        # padded_features_2 = pad_for_grid(f_2, self.GS)
        # bg_protos_2 = self.get_bg_prototypes(padded_features_2, padded_mask_2, grid_bg_2, self.GS)  # (8,16,512,1,1)
        # fg_protos_2 = self.BAP(f_2, bboxes, batchID_of_box, bg_protos_2, valid_cellIDs_2, ind_valid_bg_mask,
        #                        GAP)  # (8,512,2,2)
        # ##########################################################
        # num_fgs_2 = fg_protos_2.shape[0]  # 8
        # fg_protos_2 = fg_protos_2.view(num_fgs_2, dims, -1).permute(0, 2, 1).contiguous().view(-1, dims, 1,
        #                                                                                        1)  # (32,512,1,1) # (num_fgs,dims,OH,OW) --> (num_fgs*OH*OW,dims,1,1)
        # bg_protos_2 = bg_protos_2.contiguous().view(-1, dims, 1, 1)[
        #     valid_gridIDs_2]  # (117,512,1,1) (len(valid_gridIDs),dims,1,1)
        # protos_2 = torch.cat((fg_protos_2, bg_protos_2), dim=0)  # (149,512,1,1)

        # batch_size, dims, fH, fW = f_3.shape
        # ##########################################################
        # padded_mask_3 = pad_for_grid(F.interpolate(bg_mask, (fH, fW)), self.GS)  # (8,1,16,16)
        # grid_bg_3, valid_gridIDs_3 = self.get_grid_bg_and_IDs(padded_mask_3, self.GS)  # 8
        # valid_cellIDs_3 = []
        # for grids in grid_bg_3:
        #     valid_cellIDs_3.append([idx for idx, cell in enumerate(grids) if cell > 0])
        # ##########################################################
        # padded_features_3 = pad_for_grid(f_3, self.GS)
        # bg_protos_3 = self.get_bg_prototypes(padded_features_3, padded_mask_3, grid_bg_3, self.GS)  # (8,16,512,1,1)
        # fg_protos_3 = self.BAP(f_3, bboxes, batchID_of_box, bg_protos_3, valid_cellIDs_3, ind_valid_bg_mask,
        #                        GAP)  # (8,512,2,2)
        # ##########################################################
        # num_fgs_3 = fg_protos_3.shape[0]  # 8
        # fg_protos_3 = fg_protos_3.view(num_fgs_3, dims, -1).permute(0, 2, 1).contiguous().view(-1, dims, 1,
        #                                                                                        1)  # (32,512,1,1) # (num_fgs,dims,OH,OW) --> (num_fgs*OH*OW,dims,1,1)
        # bg_protos_3 = bg_protos_3.contiguous().view(-1, dims, 1, 1)[
        #     valid_gridIDs_3]  # (117,512,1,1) (len(valid_gridIDs),dims,1,1)
        # protos_3 = torch.cat((fg_protos_3, bg_protos_3), dim=0)  # (149,512,1,1)

        # protos_he = (protos_2 + protos_3) / 2
        protos = protos_1



        out = self.classifier(protos)#(149,2,1,1)
        return out
