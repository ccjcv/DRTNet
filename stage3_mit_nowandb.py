...
# from my_model.unet import UNet
...
# from losses.JSD_loss_urpc import calc_jsd_multiscale_2 as calc_jsd_temp
...
def train(cfg, train_loader, model, checkpoint):
    ...
    for it in tqdm(range(curr_it + 1, cfg.SOLVER.MAX_ITER + 1)):
        ...
        if cfg.MODEL.LOSS == "NAL":
            ycrf = ycrf.cuda().long()
            yret = yret.cuda().long()
            classifier_weight = torch.clone(model.decode_head.linear_pred.weight.data)
            feature_map = F.interpolate(logit, (img_size[2], img_size[3]), mode='bilinear', align_corners=False)
        ...
        if cfg.MODEL.LOSS == "BS":
            ...
            ycrf = ycrf.cuda().long()
            # ygt = ygt.cuda().long()
            loss = criterion(logit, ycrf)#ycrf
        elif cfg.MODEL.LOSS == "CE_RET":
            yret = yret.cuda().long()
            loss = criterion(logit, yret)
        # elif cfg.MODEL.LOSS == "consistency loss":
        #     # loss_ce, consistency, mixture_label = calc_jsd_temp(
        #     #     weight.cuda(), ycrf.cuda().long(), logit, logit_small, logit_large, threshold=0.9)
        #     loss_ce, consistency_loss, mixture_label = calc_jsd_temp(ycrf.cuda().long(), logit, logit_small,
        #                                                              logit_large, threshold=0.9)
...
def main(cfg):
    ...
    # model = UNet(n_classes=cfg.DATA.NUM_CLASSES)
    # model = DeepLab_LargeFOV(cfg.DATA.NUM_CLASSES, is_CS=True)# Load pre-trained backbone weights
    ...
    if cfg.WANDB.CHECKPOINT:
        ...
        checkpoint = torch.load("/home/caichengjie/anaconda3/envs/torch1.10/daima/BANA-main/ckpts/23805.pth")
...
def get_args():
    ...
    #                     help="scale_factor of upsample the i"
    #                          ""
    #                          "mage")
