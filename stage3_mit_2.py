...
def train(cfg, train_loader, model, checkpoint):
    ...
    for it in tqdm(range(curr_it + 1, cfg.SOLVER.MAX_ITER + 1)):
        ...
        if cfg.MODEL.LOSS == "BS":
            ...
            loss_ce, consistency_loss, mixture_label = calc_jsd_temp(weight, ycrf.cuda().long(), logit, logit_small,
                                                                     logit_large, threshold=0.9)
            # loss = loss_ce + 1 * consistency_loss
            consistency_weight = get_current_consistency_weight(curr_it//288)
...
def val(cfg, data_loader, model, checkpoint):
    ...
    crf_accuracy, crf_iou, crf_cls_iou, dsc = dense_crf(cfg, data_loader, model)
...
def get_args():
    ...
    parser.add_argument('--consistency', type=float,
                        default=1, help='consistency')#yuanlai 0.1
